 ...
 *
 * IMPORTANTE: Este componente mantiene una estructura DOM estable para evitar
 * errores de "insertBefore" cuando React reconcilia cambios de estado rápidos.
 * En lugar de early returns con árboles DOM incompatibles, usa un solo return
 * con visibilidad condicional del contenido interno.
 */
export function FunnelLoanSummaryCard() {
  ...
  // Un solo contenedor <div> siempre presente. El contenido interno cambia
  // pero el nodo padre nunca se desmonta, evitando el error insertBefore.
  return (
    <div className="mb-6">
      {/* Estado: Loading */}
      {loading && (
      ...
      {noConfig && (
        <Card className="border-warning-200 bg-warning-50 py-0">
          ...
          <div className="bg-linear-to-br from-primary-500 to-primary-700 p-5 relative overflow-hidden">
            ...
            <div className="relative flex items-start justify-between">
              ...
              <Button
                ...
                onClick={handleEditClick}
              >
                <span className="relative w-3 h-3 mr-1.5 inline-flex items-center justify-center shrink-0">
                  <X className={cn("w-3 h-3 absolute inset-0", !isOpen && "hidden")} />
                  <Pencil className={cn("w-3 h-3 absolute inset-0", isOpen && "hidden")} />
                </span>
